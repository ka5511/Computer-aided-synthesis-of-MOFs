...
"""
# Purpose: Compare the performance of the best versions of all selected algorithms in predicting MOF shape/quality
####################################################################
...
dtc_random = DecisionTreeClassifier(random_state=0)
...
def get_models_shape():
    models = list()
    # models.append(rf_random2.best_estimator_)
...
# First two are converted to text because they include RF and RF bootsampling takes a very long time. If activated, correct (i) values
# models_list = ['RF','LR','Ridge','GPC','Meta-Stacking','RF','DTC','LSVC','GNB','Meta-Stacking']
# models_list_expanded = ['Random Forest','Logistic Regression','Ridge Classifier','Gaussian Process Classifier','Meta Learning (Stacking)','Random Forest','Decision Tree Classifier','Linear SVC','Gaussian NB','Meta Learning (Stacking)']
models_list = ['LR','Ridge','GPC','Meta-Stacking','DTC','LSVC','GNB','Meta-Stacking']
models_list_expanded = ['Logistic Regression','Ridge Classifier','Gaussian Process','Meta Learning (Stacking)','Decision Tree Classifier','Linear SVC','Gaussian NB','Meta Learning (Stacking)']
...
for model in get_models_shape():
    ...
    MCC_shape_scores.append(stats_mcc_shape)
    # plot accuracy scores
    plt.title("Accuracy Shape for "+models_list_expanded[j])
    ...
    # plot MCC scores
    plt.title("MCC Shape for "+models_list_expanded[j])
...
for model in get_models_quality():
    ...
    MCC_quality_scores.append(stats_mcc_quality)
    # plot accuracy scores
    plt.title("Accuracy Quality for "+models_list_expanded[j+4])
    ...
    # plot MCC scores
    plt.title("MCC Quality for "+models_list_expanded[j+4])
...
lists = models_list_expanded[0:4]
# Plotting all Shape algorithm results
plt.title("Box Plot for MCC Scores from Stratified Bootsampling for all Shape Algorithms")
plt.boxplot(MCC_shape_scores, labels=lists, showmeans=True)
plt.xticks(fontsize=8)
plt.show()

lists = models_list_expanded[4:8]
# Plotting all Quality algorithm results
plt.title("Box Plot for MCC Scores from Stratified Bootsampling for all Quality Algorithms")
