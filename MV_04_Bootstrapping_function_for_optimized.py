...
filename3 = "RF_Randomized_Optimized_Quality.joblib"
...
# run bootstrap for Optimized model
...
for i in range(n_iterations):
    ...
    # fit model
    model = optimized_RF_shape.best_estimator_
...
plt.title("Optimized Accuracy Shape")
plt.vlines(mean(stats_accuracy_shape), [0], 5000, lw=2.5, linestyle="-", label='Mean = '+"{:.3f}".format(mean(stats_accuracy_shape)))
plt.vlines(median(stats_accuracy_shape), [0], 5000, lw=2.5, linestyle="-", label='Median = '+"{:.3f}".format(median(stats_accuracy_shape)),color="C1")
alpha = 0.9
p = ((1.0-alpha)/2.0) * 100
lower = max(0.0, np.percentile(stats_accuracy_shape, p))
plt.vlines(lower, [0], 1000, lw=2.5, linestyle="dotted", label=str(alpha*100)+'% CI', color="C2")
p = (alpha+((1.0-alpha)/2.0)) * 100
upper = min(1.0, np.percentile(stats_accuracy_shape, p))
plt.vlines(upper, [0], 1000, lw=2.5, linestyle="dotted", color="C2")
...
# plot MCC scores
plt.title("Optimized MCC Shape")
plt.vlines(mean(stats_mcc_shape), [0], 5000, lw=2.5, linestyle="-", label='Mean = '+"{:.3f}".format(mean(stats_mcc_shape)))
plt.vlines(median(stats_mcc_shape), [0], 5000, lw=2.5, linestyle="-", label='Median = '+"{:.3f}".format(median(stats_mcc_shape)),color="C1")
p = ((1.0-alpha)/2.0) * 100
...
for i in range(n_iterations):
    ...
    # fit model
    model = optimized_RF_quality.best_estimator_
    model.fit(train[:,:-1], train[:,-1])
    # evaluate model
    predictions = model.predict(test[:,:-1])
    score_accuracy = accuracy_score(test[:,-1], predictions)
...
# plot accuracy scores
plt.title("Optimized Accuracy Quality")
plt.vlines(mean(stats_accuracy_quality), [0], 5000, lw=2.5, linestyle="-", label='Mean = '+"{:.3f}".format(mean(stats_accuracy_quality)))
plt.vlines(median(stats_accuracy_quality), [0], 5000, lw=2.5, linestyle="-", label='Median = '+"{:.3f}".format(median(stats_accuracy_quality)),color="C1")
p = ((1.0-alpha)/2.0) * 100
...
# plot MCC scores
plt.title("Optimized MCC Quality")
plt.vlines(mean(stats_mcc_quality), [0], 5000, lw=2.5, linestyle="-", label='Mean = '+"{:.3f}".format(mean(stats_mcc_quality)))
plt.vlines(median(stats_mcc_quality), [0], 5000, lw=2.5, linestyle="-", label='Median = '+"{:.3f}".format(median(stats_mcc_quality)),color="C1")
p = ((1.0-alpha)/2.0) * 100
