...
"""
# Purpose: Perform the indicated K-Fold cross-validations on optimized selected algorithms to check MOF shape/quality prediction performance. Input to these  models consists of only the first 3 principal components (PC) obtained from the PCA analysis. If data augmentation is applied, the applied technique is reflected on the file title.
####################################################################
...
filename2 = "LR_Grid_Optimized_Shape.joblib"
lr_random = joblib.load(filename2)
filename2 = "RC_Grid_Optimized_Shape.joblib"
rc_random = joblib.load(filename2)
filename2 = "GPC_Grid_Optimized_Shape.joblib"
gpc_random = joblib.load(filename2)

filename2 = "DTC_Grid_Optimized_Quality.joblib"
dtc_random = joblib.load(filename2)
filename2 = "LSVC_Grid_Optimized_Quality.joblib"
...
def get_models_shape():
 	models = list()
 	models.append(lr_random.best_estimator_)
 	models.append(rc_random.best_estimator_)
 	models.append(gpc_random.best_estimator_)
 	return models

def get_models_quality():
 	models = list()
 	models.append(dtc_random.best_estimator_)
 	models.append(lsvc_random.best_estimator_)
 	models.append(gnb_random.best_estimator_)
 	return models
...
models_list_expanded = ['Grid Optimized Logistic Regression','Grid Optimized Ridge Classifier','Grid Optimized Gaussian Process Classifier','Grid Optimized Decision Tree Classifier','Grid Optimized Linear SVC','Grid Optimized Gaussian NB']
models_results_shape = list()
splits = 3
i=0
for model in get_models_shape():
    models_results_shape.append(cross_validation(model, principal_components, encoded_y2, RepeatedKFold(n_splits = splits,random_state=1)))
    plot_result(models_list_expanded[i]+"Repeated K-Fold", "Accuracy", "MOF Shape Accuracy Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Accuracy scores"], models_results_shape[i]["Validation Accuracy scores"], models_results_shape[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold", "Precision", "MOF Shape Precision Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Precision scores"], models_results_shape[i]["Validation Precision scores"], models_results_shape[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold", "Recall", "MOF Shape Recall Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Recall scores"], models_results_shape[i]["Validation Recall scores"], models_results_shape[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold", "F1", "MOF Shape F1 Scores in "+str(splits)+" Folds", models_results_shape[i]["Training F1 scores"], models_results_shape[i]["Validation F1 scores"], models_results_shape[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i]+"Repeated K-Fold", "MCC", "MOF Shape MCC Scores in "+str(splits)+" Folds", models_results_shape[i]["MCC"], models_results_shape[i]["MCC"].mean())
    ...
    for k in folds:
        ...
        cv = RepeatedKFold(n_splits=k, random_state=1)
    ...
    # show the plot
    plt.title("Repeated KFold MOF Shape Accuracy Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    ...
    # show the plot
    plt.title("Repeated KFold MOF Shape Precision Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold MOF Shape Recall Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold MOF Shape F1 Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold MOF Shape MCC Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
...
for model in get_models_quality():
    models_results_quality.append(cross_validation(model, principal_components, encoded_y3, RepeatedKFold(n_splits=splits, random_state=1)))
    plot_result(models_list_expanded[i+3]+"Repeated KFold", "Accuracy", "Repeated KFold MOF Quality Accuracy Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Accuracy scores"], models_results_quality[i]["Validation Accuracy scores"], models_results_quality[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold", "Precision", "Repeated KFold MOF Quality Precision Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Precision scores"], models_results_quality[i]["Validation Precision scores"], models_results_quality[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold", "Recall", "Repeated KFold MOF Quality Recall Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Recall scores"], models_results_quality[i]["Validation Recall scores"], models_results_quality[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold", "F1", "Repeated KFold MOF Quality F1 Scores in "+str(splits)+" Folds", models_results_quality[i]["Training F1 scores"], models_results_quality[i]["Validation F1 scores"], models_results_quality[i]["Mean Validation F1 Score"])
    ...
    plt.plot(folds, [a_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold MOF Quality Accuracy Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Accuracy Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [p_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold MOF Quality Precision Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Precision Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [r_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold MOF Quality Recall Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Recall Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [f_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold MOF Quality F1 Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("F1 Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.errorbar(folds, mat_means, yerr=[mat_mins, mat_maxs], fmt='o')
    # show the plot
    plt.title("Repeated KFold MOF Quality MCC Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("MCC Score", fontsize=14)
    plt.grid()
    plt.show()
