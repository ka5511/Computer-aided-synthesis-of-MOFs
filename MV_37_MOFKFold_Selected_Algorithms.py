...
"""
# Purpose: Perform the indicated K-Fold cross-validations on selected algorithms to check MOF shape/quality prediction performance. Input to these models consists of all features available in the dataset.
####################################################################
...
for model in get_models_shape():
    models_results_shape.append(cross_validation(model, X, encoded_y2, KFold(n_splits = splits,shuffle=True,random_state=1)))
    plot_result(models_list_expanded[i], "Accuracy", "MOF Shape Accuracy Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Accuracy scores"], models_results_shape[i]["Validation Accuracy scores"], models_results_shape[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i], "Precision", "MOF Shape Precision Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Precision scores"], models_results_shape[i]["Validation Precision scores"], models_results_shape[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i], "Recall", "MOF Shape Recall Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Recall scores"], models_results_shape[i]["Validation Recall scores"], models_results_shape[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i], "F1", "MOF Shape F1 Scores in "+str(splits)+" Folds", models_results_shape[i]["Training F1 scores"], models_results_shape[i]["Validation F1 scores"], models_results_shape[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i], "MCC", "MOF Shape MCC Scores in "+str(splits)+" Folds", models_results_shape[i]["MCC"], models_results_shape[i]["MCC"].mean())
    ideal_values = cross_validation(model,X,encoded_y2,LeaveOneOut())
    ...
    for k in folds:
        ...
        results = cross_validation(model, X, encoded_y2, cv)
        # Evaluate k values
    ...
    # show the plot
    plt.title("MOF Shape Accuracy Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    ...
    # show the plot
    plt.title("MOF Shape Precision Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("MOF Shape Recall Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("MOF Shape F1 Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("MOF Shape MCC Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
...
for model in get_models_quality():
    models_results_quality.append(cross_validation(model, X, encoded_y3, KFold(n_splits=splits, shuffle=True,random_state=1)))
    plot_result(models_list_expanded[i+3], "Accuracy", "MOF Quality Accuracy Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Accuracy scores"], models_results_quality[i]["Validation Accuracy scores"], models_results_quality[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i+3], "Precision", "MOF Quality Precision Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Precision scores"], models_results_quality[i]["Validation Precision scores"], models_results_quality[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i+3], "Recall", "MOF Quality Recall Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Recall scores"], models_results_quality[i]["Validation Recall scores"], models_results_quality[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i+3], "F1", "MOF Quality F1 Scores in "+str(splits)+" Folds", models_results_quality[i]["Training F1 scores"], models_results_quality[i]["Validation F1 scores"], models_results_quality[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i+3], "MCC", "MOF Quality MCC Scores in "+str(splits)+" Folds", models_results_quality[i]["MCC"], models_results_quality[i]["MCC"].mean())
    ideal_values = cross_validation(model,X,encoded_y3,LeaveOneOut())
    a_ideal = ideal_values["Mean Validation Accuracy"]
    p_ideal = ideal_values["Mean Validation Precision"]
    r_ideal = ideal_values["Mean Validation Recall"]
    f_ideal = ideal_values["Mean Validation F1 Score"]
    ...
    for k in folds:
        # Define test conditions
        cv = KFold(n_splits=k, shuffle=True, random_state=1)
        results = cross_validation(model, X, encoded_y3, cv)
        # Evaluate k values
        k_mean = results["Mean Validation Accuracy"]
        k_min = results["Validation Accuracy scores"].min()
        k_max = results["Validation Accuracy scores"].max()
    ...
    plt.plot(folds, [a_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("MOF Quality Accuracy Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Accuracy Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [p_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("MOF Quality Precision Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Precision Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [r_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("MOF Quality Recall Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Recall Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [f_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("MOF Quality F1 Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("F1 Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.errorbar(folds, mat_means, yerr=[mat_mins, mat_maxs], fmt='o')
    # show the plot
    plt.title("MOF Quality MCC Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("MCC Score", fontsize=14)
    plt.grid()
    plt.show()
