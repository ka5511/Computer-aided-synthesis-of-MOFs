...
"""
# Purpose: Perform exhaustive grid search over specified parameter values using stratified K-fold for cross-validation
####################################################################
...
n_estimators = [int(x) for x in np.linspace(start = 400, stop = 2000, num = 16)]
# Maximum number of levels in tree
max_depth = [int(x) for x in np.linspace(60, 110, num = 5)]
...
rf_2_random = GridSearchCV(estimator = rf, param_grid = random_grid, cv = splits, scoring=make_scorer(matthews_corrcoef), n_jobs = None, error_score='raise')
# Fit the random search model
best_random_model_2 = rf_2_random.fit(train_features2, train_labels2)

filename2 = "RF_Grid_Optimized_Shape2.joblib"
...
# search across 100 different combinations
rf = RandomForestClassifier(random_state=1)
rf_3_random = GridSearchCV(estimator = rf, param_grid = random_grid, cv = splits, scoring=make_scorer(matthews_corrcoef), n_jobs = None, error_score='raise')
# Fit the random search model
best_random_model_3 = rf_3_random.fit(train_features3, train_labels3)

filename3 = "RF_Grid_Optimized_Quality2.joblib"
