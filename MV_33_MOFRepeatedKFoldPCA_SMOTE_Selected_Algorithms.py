...
"""
# Purpose: Perform the indicated K-Fold cross-validations on selected algorithms to check MOF shape/quality prediction performance. Input to the random forest model consists of only the first 3 principal components (PC) obtained from the PCA analysis. If data augmentation is applied, the applied technique is reflected on the file title.
####################################################################
...
models_list_expanded = ['Logistic Regression','Ridge Classifier','Gaussian Process Classifier','Decision Tree Classifier','Linear SVC','Gaussian NB']
...
for model in get_models_shape():
    ...
    # Use pipeline
    models_results_shape.append(cross_validation(pipeline, principal_components, encoded_y2, RepeatedKFold(n_splits = splits, random_state=1)))
    plot_result(models_list_expanded[i]+"Repeated K-Fold with SMOTE", "Accuracy", "MOF Shape Accuracy Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Accuracy scores"], models_results_shape[i]["Validation Accuracy scores"], models_results_shape[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold with SMOTE", "Precision", "MOF Shape Precision Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Precision scores"], models_results_shape[i]["Validation Precision scores"], models_results_shape[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold with SMOTE", "Recall", "MOF Shape Recall Scores in "+str(splits)+" Folds", models_results_shape[i]["Training Recall scores"], models_results_shape[i]["Validation Recall scores"], models_results_shape[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i]+"Repeated K-Fold with SMOTE", "F1", "MOF Shape F1 Scores in "+str(splits)+" Folds", models_results_shape[i]["Training F1 scores"], models_results_shape[i]["Validation F1 scores"], models_results_shape[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i]+"Repeated K-Fold with SMOTE", "MCC", "MOF Shape MCC Scores in "+str(splits)+" Folds", models_results_shape[i]["MCC"], models_results_shape[i]["MCC"].mean())
    ...
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Shape Accuracy Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    ...
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Shape Precision Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Shape Recall Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Shape F1 Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    ...
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Shape MCC Scores for Different K Values"+models_list_expanded[i], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
...
for model in get_models_quality():
    ...
    pipeline = Pipeline(steps=steps)
    # Use pipeline    
    models_results_quality.append(cross_validation(pipeline, principal_components, encoded_y3, RepeatedKFold(n_splits=splits, random_state=1)))
    plot_result(models_list_expanded[i+3]+"Repeated KFold with SMOTE", "Accuracy", "Repeated KFold MOF Quality Accuracy Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Accuracy scores"], models_results_quality[i]["Validation Accuracy scores"], models_results_quality[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold with SMOTE", "Precision", "Repeated KFold MOF Quality Precision Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Precision scores"], models_results_quality[i]["Validation Precision scores"], models_results_quality[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold with SMOTE", "Recall", "Repeated KFold MOF Quality Recall Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Recall scores"], models_results_quality[i]["Validation Recall scores"], models_results_quality[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i+3]+"Repeated KFold with SMOTE", "F1", "Repeated KFold MOF Quality F1 Scores in "+str(splits)+" Folds", models_results_quality[i]["Training F1 scores"], models_results_quality[i]["Validation F1 scores"], models_results_quality[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i+3]+"Repeated KFold with SMOTE", "MCC", "Repeated KFold MOF Quality MCC Scores in "+str(splits)+" Folds", models_results_quality[i]["MCC"], models_results_quality[i]["MCC"].mean())
    ...
    plt.plot(folds, [a_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Quality Accuracy Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Accuracy Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [p_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Quality Precision Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Precision Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [r_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Quality Recall Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("Recall Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.plot(folds, [f_ideal for _ in range(len(folds))], color='r',label="LOOCV Score")
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Quality F1 Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("F1 Score", fontsize=14)
    plt.grid()
    plt.legend(loc='lower left')
    ...
    plt.errorbar(folds, mat_means, yerr=[mat_mins, mat_maxs], fmt='o')
    # show the plot
    plt.title("Repeated KFold with SMOTE MOF Quality MCC Scores for Different K Values"+models_list_expanded[i+3], fontsize=15)
    plt.xlabel("K Value", fontsize=14)
    plt.ylabel("MCC Score", fontsize=14)
    plt.grid()
    plt.show()
