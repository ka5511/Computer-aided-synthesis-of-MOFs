...
"""
# Purpose: Perform bootstrap sampling to train selected algorithms (either base or optimized version) and check MOF shape/quality prediction performance. Input to these  models consists of only the first 3 principal components (PC) obtained from the PCA analysis. If data augmentation is applied, the applied technique is reflected on the file title.
####################################################################
...
for model in get_models_quality():
    ...
    # plot accuracy scores
    plt.title("Accuracy Quality for "+models_list_expanded[j])
    ...
    # plot MCC scores
    plt.title("MCC Quality for "+models_list_expanded[j])
