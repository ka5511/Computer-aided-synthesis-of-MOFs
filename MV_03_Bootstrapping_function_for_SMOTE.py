...
for i in range(n_iterations):
    ...
    # fit model
    model = RandomForestClassifier(criterion="entropy",min_samples_split=2,random_state=0)
    # Define a pipeline
    steps = [('over', BorderlineSMOTE(k_neighbors=1)), ('model', model)]
    pipeline = Pipeline(steps=steps)
    pipeline.fit(train[:,:-1], train[:,-1])
    # evaluate model
    predictions = pipeline.predict(test[:,:-1])
...
plt.title("SMOTE Accuracy Shape")
...
# plot accuracy scores
plt.title("SMOTE Accuracy Quality")
