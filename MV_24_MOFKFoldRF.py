...
"""
# Purpose: Perform the indicated K-Fold cross-validations on a random forest algorithms to check MOF shape/quality prediction performance. Input to the random forest model consists of all features available in the dataset
####################################################################
...
K_clf_model2_result = cross_validation(K_clf_model2, X, encoded_y2, KFold(n_splits=splits, shuffle=True,random_state=1))
...
ideal_values = cross_validation(K_clf_model2,X,encoded_y2,LeaveOneOut())
...
for k in folds:
    ...
    results = cross_validation(K_clf_model2, X, encoded_y2, cv)
    # Evaluate k values
...
K_clf_model3_result = cross_validation(K_clf_model3, X, encoded_y3, KFold(n_splits=splits, shuffle=True,random_state=1))
...
ideal_values = cross_validation(K_clf_model3,X,encoded_y3,LeaveOneOut())
a_ideal = ideal_values["Mean Validation Accuracy"]
stddev = std(ideal_values["Validation Accuracy scores"])
print('Accuracy: %.3f (%.3f)' % (a_ideal, stddev))
print('Ideal: %.3f' % a_ideal)
...
for k in folds:
    # Define test conditions
    cv = KFold(n_splits=k, shuffle=True, random_state=1)
    results = cross_validation(K_clf_model3, X, encoded_y3, cv)
    # Evaluate k values
    k_mean = results["Mean Validation Accuracy"]
    k_min = results["Validation Accuracy scores"].min()
    k_max = results["Validation Accuracy scores"].max()
