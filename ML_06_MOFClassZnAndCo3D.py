...
"""
# Purpose: Perform supervised classification modeling for MOFs shape and quality using Random Forest and Logistic Regression algorithms and plotting results in 3-d PC space
####################################################################
...
ax.contour3D(X1, X2, X3, clf_2_scaled.predict(np.array([X1.ravel(),
 			X2.ravel(), X3.ravel()]).T).reshape(X1.shape), alpha = 0.75,
 			cmap = ListedColormap(('#326da8','#a88932')))

ax.xlim(X1.min(), X1.max())
ax.ylim(X2.min(), X2.max())
ax.zlim(X3.min(), X3.max())

legend_2 = ["Monolith","Powder"]

for i, j in enumerate(np.unique(y2_set)):
 	plt.scatter(X_set[y2_set == j, 0], X_set[y2_set == j, 1], X_set[y2_set == j, 2],
...
for i, j in enumerate(np.unique(y1_set)):
 	plt.scatter(X_set[y1_set == j, 0], X_set[y1_set == j, 1], X_set[y1_set == j, 2],
...
ax.contour3D(X1, X2, X3, classifier_hardness.predict(np.array([X1.ravel(),
 			X2.ravel(), X3.ravel()]).T).reshape(X1.shape), alpha = 0.75,
 			cmap = ListedColormap(('#326da8','#a88932')))
...
ax.contour3D(X1, X2, X3, clf_3_scaled.predict(np.array([X1.ravel(),
 			X2.ravel(), X3.ravel()]).T).reshape(X1.shape), alpha = 0.75,
...
for i, j in enumerate(np.unique(y2_set)):
 	ax.scatter(X_set[y2_set == j, 0], X_set[y2_set == j, 1], X_set[y2_set == j, 2],
...
ax.contour3D(X1, X2, X3, classifier_both.predict(np.array([X1.ravel(),
 			X2.ravel(),X3.ravel()]).T).reshape(X1.shape), alpha = 0.75,
 			cmap = ListedColormap(('#326da8','#a88932', '#32a863')))
