...
# Import the "Most" optimized models
filename2 = "RF_Randomized_Optimized_Shape.joblib"
rf_random1 = joblib.load(filename2)

lr_random = LogisticRegression(random_state=0)

rc_random = RidgeClassifier(random_state=0)
filename2 = "GPC_Randomized_Optimized_Shape.joblib"
gpc_random = joblib.load(filename2)
filename2 = "Meta_Shape.joblib"
meta_model2 = joblib.load(filename2)

# Define a pipeline
steps = [('over', BorderlineSMOTE(k_neighbors=1)), ('model', RandomForestClassifier(random_state=0))]
pipeline = Pipeline(steps=steps)
rf_random2 = pipeline
# Define a pipeline
steps = [('over', BorderlineSMOTE(k_neighbors=1)), ('model', DecisionTreeClassifier(random_state=0))]
pipeline = Pipeline(steps=steps)
dtc_random = pipeline
...
meta_model3 = joblib.load(filename2)


def get_models_shape():
    models = list()
    models.append(rf_random1.best_estimator_)
    models.append(lr_random)
    models.append(rc_random)
    models.append(gpc_random.best_estimator_)
    models.append(meta_model2)
    return models

def get_models_quality():
    models = list()
    models.append(rf_random2)
    models.append(dtc_random)
    models.append(lsvc_random.best_estimator_)
    models.append(gnb_random.best_estimator_)
    models.append(meta_model3)
    return models
...
models_list = ['RF','LR','Ridge','GPC','Meta-Stacking','RF','DTC','LSVC','GNB','Meta-Stacking']
models_list_expanded = ['Random Forest','Logistic Regression','Ridge Classifier','Gaussian Process','Meta Learning','Random Forest','Decision Tree Classifier','Linear SVC','Gaussian NB','Meta Learning (Stacking)']
models_results_shape = list()
splits = 3
i=0
MCC_shape_scores = list()
for model in get_models_shape():
    models_results_shape.append(cross_validation(model, principal_components, encoded_y2, RepeatedStratifiedKFold(n_splits = splits,random_state=1)))
    ...
    MCC_shape_scores.append(models_results_shape[i]["MCC"])
...
for model in get_models_quality():
    models_results_quality.append(cross_validation(model, principal_components, encoded_y3, RepeatedStratifiedKFold(n_splits=splits, random_state=1)))
    plot_result(models_list_expanded[i+5]+"Repeated Stratified KFold", "Accuracy", "MOF Quality Accuracy Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Accuracy scores"], models_results_quality[i]["Validation Accuracy scores"], models_results_quality[i]["Mean Validation Accuracy"])
    plot_result(models_list_expanded[i+5]+"Repeated Stratified KFold", "Precision", "MOF Quality Precision Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Precision scores"], models_results_quality[i]["Validation Precision scores"], models_results_quality[i]["Mean Validation Precision"])
    plot_result(models_list_expanded[i+5]+"Repeated Stratified KFold", "Recall", "MOF Quality Recall Scores in "+str(splits)+" Folds", models_results_quality[i]["Training Recall scores"], models_results_quality[i]["Validation Recall scores"], models_results_quality[i]["Mean Validation Recall"])
    plot_result(models_list_expanded[i+5]+"Repeated Stratified KFold", "F1", "MOF Quality F1 Scores in "+str(splits)+" Folds", models_results_quality[i]["Training F1 scores"], models_results_quality[i]["Validation F1 scores"], models_results_quality[i]["Mean Validation F1 Score"])
    plot_result_MCC(models_list_expanded[i+5]+"Repeated Stratified KFold", "MCC", "MOF Quality MCC Scores in "+str(splits)+" Folds", models_results_quality[i]["MCC"], models_results_quality[i]["MCC"].mean())
    MCC_quality_scores.append(models_results_quality[i]["MCC"])
...
lists = models_list[0:5]
# Plotting all Shape algorithm results
plt.title("Box Plot for MCC Scores for all Shape Algorithms")
plt.boxplot(MCC_shape_scores, labels=lists, showmeans=True)
plt.xticks(fontsize=8)
plt.show()

lists = models_list[5:10]
# Plotting all Quality algorithm results
plt.title("Box Plot for MCC Scores for all Quality Algorithms")
plt.boxplot(MCC_quality_scores, labels=lists, showmeans=True)
plt.xticks(fontsize=8)
plt.show()
